...
- `packages/@itsrakesh/ui`: UI components
...
You'll need to create an `NPM_TOKEN` and `GH_TOKEN` and add it to your GitHub repository settings to enable access to npm. It's also worth installing the [Changesets bot](https://github.com/apps/changeset-bot) on your repository.
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `itsrakesh` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `itsrakesh` with your desired scope
- Search and replace `itsrakesh` with your desired scope
- Re-run `pnpm install`
